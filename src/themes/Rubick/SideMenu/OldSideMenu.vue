...
<template>
  ...
  >
    <MobileMenu />
    <div class="mt-[4.7rem] flex md:mt-0 rtwtwrtwrt">
      <!-- BEGIN: Side Menu -->
      <nav class="side-nav hidden w-[80px] overflow-x-hidden pb-16 pr-5 md:block xl:w-[230px]">
        <RouterLink :to="{ name: 'dashboard-overview-1' }" class="flex items-center pt-4 pl-5 intro-x">
          <img alt="Jaldi Logo" class="w-28" :src="logoUrl"/>
        </RouterLink>
        ...
        <ul>
          <template v-for="(menu, menuKey) in formattedMenu">
            ...
            <li v-else :key="menuKey">
              <Tippy as="a" :content="menu.title" :options="{  placement: 'right',  }"
                :disable="windowWidth > 1260"
                :href=" menu.subMenu ? '#' : ((pageName: string | undefined) => {
                        try {
