<template>
  <div class="flex items-center mt-8 intro-y" :class="{ 'flex-row-reverse': locale === 'ar' }">
    ...
    <h2 class="text-lg font-medium flex-1" :class="locale === 'ar' ? 'text-right ml-auto' : 'mr-auto'">{{ $t('superadmin.editShop.title') }}</h2>
  </div>
  <div class="intro-y box mt-5">

    <div v-if="isLoading" class="p-5 text-center">
      <LoadingIcon icon="oval" class="w-8 h-8 mx-auto" />
      <p class="mt-2 text-gray-600">{{ $t('superadmin.editShop.loadingShopDetails') }}</p>
    </div>

    <form v-else @submit.prevent="handleSubmit" class="p-5">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        <!-- Shop Name -->
        <div class="md:col-span-6">
          <FormLabel>{{ $t('superadmin.editShop.shopName') }} *</FormLabel>
          <FormInput
            v-model="formData.name"
            type="text"
            :placeholder="$t('superadmin.editShop.shopNamePlaceholder')"
        ...
        <!-- Email -->
        <div class="md:col-span-6">
          <FormLabel>{{ $t('superadmin.editShop.email') }} *</FormLabel>
          <FormInput
            v-model="formData.email"
            type="email"
            :placeholder="$t('superadmin.editShop.emailPlaceholder')"
        ...
        <div class="md:col-span-6 relative">
          <FormLabel>{{ $t('superadmin.editShop.password') }}</FormLabel>
          <FormInput
            v-model="formData.password"
            :type="isPasswordVisible ? 'text' : 'password'"
            :placeholder="$t('superadmin.editShop.passwordPlaceholder')"
        ...
        <div v-if="formData.password" class="md:col-span-6 relative">
          <FormLabel>{{ $t('superadmin.editShop.confirmPassword') }} *</FormLabel>
          <FormInput
            v-model="formData.password_confirmation"
            :type="isConfirmPasswordVisible ? 'text' : 'password'"
            :placeholder="$t('superadmin.editShop.confirmPasswordPlaceholder')"
        ...
        <!-- Phone -->
        <div class="md:col-span-6">
          <FormLabel>{{ $t('superadmin.editShop.phone') }}</FormLabel>
        ...
        <!-- Address -->
        <div class="md:col-span-6">
          <FormLabel>{{ $t('superadmin.editShop.address') }}</FormLabel>
          <FormTextarea
            v-model="formData.address"
            :placeholder="$t('superadmin.editShop.addressPlaceholder')"
            rows="3"
      ...
      <div class="flex items-center justify-end gap-3 mt-8 pt-5 border-t border-slate-200/60">
        ...
        >
          ...
          <span v-if="isSubmitting">{{ $t('superadmin.editShop.updating') }}</span>
          <span v-else>{{ $t('superadmin.editShop.update') }}</span>
        </Button>
      </div>
    </form>
  </div>
...
const superAdminId = computed(() => authStore.getUserId || authStore.user?.id || '');
...
const fetchShop = async () => {
  ...
  try {
    const response = await superAdminApi.getShop({
      id: shopId,
      superAdminId: superAdminId.value,
    });
    const result = handleResponse(response);
    ...
    if (result.success) {
      ...
      if (existingPhone) {
        ...
        if (numbers.startsWith('966') && numbers.length > 3) {
          ...
          const cleanPhone = numbers.replace(/^0+/, '');
          const countryCodeNum = existingCountryCode.replace(/[^0-9]/g, '');
          formattedPhone = `+${countryCodeNum}${cleanPhone}`;
        }
      }
...
const handleSubmit = async () => {
  if (!superAdminId.value) {
    toast().fry(pan.error(t('superadmin.editShop.superAdminIdRequired')));
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      toast().fry(pan.success(t('superadmin.editShop.shopUpdatedSuccess')));
      router.push({ name: 'SuperAdminShops' });
    } else {
      if (result.errors) {
        errors.value = result.errors;
      } else {
        toast().fry(pan.error(result.message || t('superadmin.editShop.failedToUpdate')));
      }
    }
  } catch (error) {
    ...
    if (errorResult.errors) {
      ...
      toast().fry(pan.error(errorResult.message || t('superadmin.editShop.failedToUpdate')));
    }
  } finally {
