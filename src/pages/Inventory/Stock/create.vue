...
import InventoryStockCount from "@/components/InventoryStockCount/StockCountComponent.vue";
...
<template>
  <div class="flex items-center mt-8 intro-y">
    ...
    <h2 class="mr-auto text-lg font-medium">{{ t('stock-count.createNew') }}</h2>
  </div>
      ...
      <div class="intro-y box p-8">
        <div class="grid grid-cols-12 gap-6">
          <div class="col-span-6">
            ...
            <FormInput v-model="createStockCountForm.count_name" id="count_name" type="text" :placeholder="t('stock-count.countName')" :class="{'border-red-500': form.invalid('count_name')}" />
            <template v-if="form.invalid('count_name')">
              <div class="mt-2 text-xs text-red-600">{{ form.getError('count_name') }}</div>
            </template>
          </div>
          <div class="col-span-12">
            <FormLabel>{{ t('stock-count.notes') }}</FormLabel>
            <FormTextarea v-model="createStockCountForm.notes" class="py-2 pl-4 resize-none" :rows="3" :placeholder="t('stock-count.notes')" :class="{'border-red-500': form.invalid('notes')}" />
            <template v-if="form.invalid('notes')">
              <div class="mt-2 text-xs text-red-600">{{ form.getError('notes') }}</div>
            </template>
          </div>
          <div class="col-span-12">
            <InventoryStockCount :createStockCountForm="createStockCountForm" :form="form" />
          </div>
        </div>
      </div>
    </div>
