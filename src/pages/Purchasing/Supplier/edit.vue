...
// Format phone number from API response (966XXXXXXXXX) to display format
const formatPhoneFromApi = (phoneValue) => {
...
const fetchSupplier = async () => {
  try {
    ...
    const response = await httpClient.get(`${import.meta.env.VITE_PUBLIC_SUPPLIER_API_URL}/suppliers/${Uuid}`, {
      params: { user_id: USER_ID }
    })
    // const response = await apiService.supplier.show(Uuid);
    const result = handleResponse(response);

    if (result.success) {
      isloading.value = false;
      createFormData.value = {...result.data.data, _method: "PUT"}
    ...
    handleError(error);
    toast().fry(pan.supplier.error(error))
    isloading.value = false;
  }
}
...
const submitEditSupplier = async () => {
  try {
    ...
    // const response = await apiService.supplier.edit(route.params.uuid, payload);
    const result = handleResponse(response);

    if (result.success) {
      toast().fry(pan.supplier.success(result.message))
      console.log("submit-supplier", result)
...
<template>
  <div class="flex items-center mt-8 intro-y">
    ...
    <h2 class="mr-auto text-lg font-semibold">{{ $t('suppliers.editSupplier') }}</h2>
  </div>
      ...
      <div class="intro-y box p-8">
        <div class="grid grid-cols-12 gap-6">
          <div class="col-span-6">
            ...
            <FormInput v-model="createFormData.supplier_name" id="name_en" type="text"
                       :placeholder="$t('suppliers.supplier')"
          ...
          <div class="col-span-6">
            ...
            <FormInput v-model="createFormData.vat" id="vat_number" type="text" :placeholder="$t('suppliers.vatNumber')"
          ...
          <div class="col-span-12">
            <Button variant="primary" class="mr-2 mt-2 shadow-md" @click="submitEditSupplier">
              {{ $t('suppliers.submit') }}
            </Button>
