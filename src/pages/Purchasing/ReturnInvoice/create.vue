...
import ReturnInvoicesComponent from "@/components/PurchaseReturnInvoices/InvoicesComponent.vue";
...
// console.log("props", props.isProductBankData)
...
<template>
  ...
  <div class="flex items-center mt-8 intro-y">
    <RouterLink to="/purchasing/invoice/return">
      <Button variant="primary" size="sm" class="shadow-md mr-4 h-8 w-9">
        <Lucide icon="ChevronLeft" class="w-5 h-5   "/>
      </Button>
    </RouterLink>
    <h2 class="mr-auto text-lg font-semibold">{{ $t('purchase-returns.heading') }}</h2>
  </div>
      ...
      <div class="intro-y box p-8">
        <div class="grid grid-cols-12 gap-6">
          <div class="col-span-6">
            <FormLabel>{{ $t('purchase-returns.suppliers') }} <span class="text-danger "> *</span></FormLabel>
          ...
          <div class="col-span-6">
            <FormLabel for="supplier_number">{{$t('purchase-returns.supplierInvoiceNumber')}}</FormLabel>
            <FormInput v-model="CreateSupplierFields.supplier_invoice_number" id="supplier_number" type="text"
                       placeholder="Supplier Invoice Number"
                       :class="{'border-red-500': form.invalid('supplier_invoice_number')}"/>
          ...
          <div class="col-span-12">
            <ReturnInvoicesComponent :SupplierFields="CreateSupplierFields" :form="form"/>
          </div>
