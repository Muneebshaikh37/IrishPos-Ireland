...
const fetchSuperAdmin = async () => {
  ...
  try {
    const res = await superAdminApi.getSuperAdmin(id);
    const result = handleResponse(res);
    if (result.success) {
      const sa = result.data?.data || result.data;
      form.value.name = sa.name || '';
      form.value.email = sa.email || '';
      form.value.phone = sa.account?.phone || '';
      form.value.address = sa.account?.address || '';
      form.value.package_id = sa.current_subscription?.package?.id || '';
    } else {
      toast().fry(pan.error('Failed to load Super Admin data.'));
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.value.address !== undefined) payload.address = form.value.address;

    const res = await superAdminApi.updateSuperAdmin(id, payload);
    const result = handleResponse(res);

    if (result.success) {
      toast().fry(pan.success('Super Admin updated successfully.'));
      router.push({ name: 'ProductOwnerSuperAdmins' });
    } else {
      toast().fry(pan.error(result.message || 'Failed to update Super Admin.'));
    }
  } catch (e: any) {
...
onMounted(async () => {
  await Promise.all([fetchPackages(), fetchSuperAdmin()]);
});
</script>
...
<template>
  <div class="mt-6 intro-y max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <h2 class="text-lg font-medium">Edit Super Admin</h2>
    </div>
    ...
    <form v-else @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div class="box p-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input v-model="form.name" type="text" class="form-control w-full" :class="{ 'border-red-400': errors.name }" />
          ...
          <div>
            ...
            <input v-model="form.email" type="email" class="form-control w-full" :class="{ 'border-red-400': errors.email }" />
          ...
          <div>
            ...
            <input v-model="form.password" type="password" class="form-control w-full" :class="{ 'border-red-400': errors.password }" placeholder="Min. 8 characters" />
          ...
          <div>
            ...
            <input v-model="form.password_confirmation" type="password" class="form-control w-full" :class="{ 'border-red-400': errors.password_confirmation }" placeholder="Re-enter new password" />
          ...
          <div>
            ...
            <input v-model="form.phone" type="text" class="form-control w-full" placeholder="+353 87 000 0000" />
          </div>

          <!-- Address -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
            <input v-model="form.address" type="text" class="form-control w-full" placeholder="Business address" />
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-end gap-3 pb-6">
        <Button type="button" variant="outline-secondary" @click="router.push({ name: 'ProductOwnerSuperAdmins' })" :disabled="isSubmitting">
          Cancel
        </Button>
        <Button type="submit" variant="primary" :disabled="isSubmitting || packages.length === 0" class="flex items-center gap-2">
