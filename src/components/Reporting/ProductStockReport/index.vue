...
<template>
  <div class="p-5 intro-y box h-full">
    <div class="mb-2 flex justify-between items-center">
      <h4 class="text-lg font-semibold text-gray-800 mb-4">{{ $t('reporting.productsStockValue') }}</h4>
      <Button 
    ...
    <div v-else-if="ProductInventoryValue?.length > 0">  
      <apexchart v-if="showChart" width="100%" height="auto" type="pie" :options="options" :series="series" />
      <div v-else class="overflow-x-auto">
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-gray-50 border-b">
              <th class="text-left py-3 px-4 font-semibold text-gray-700">{{ $t('reporting.product_name') }}</th>
              <th class="text-right py-3 px-4 font-semibold text-gray-700">{{ $t('reporting.stockValue') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in ProductInventoryValue" :key="index" class="border-b hover:bg-gray-50">
              <td class="py-3 px-4 text-gray-800">{{ item.name_en }}</td>
              <td class="py-3 px-4 text-right font-medium text-gray-800">{{ CurrencyFormatter(item.inventory_value) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
