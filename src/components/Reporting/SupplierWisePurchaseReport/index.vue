...
const updateChartData = (data: any) => {
  ...
  const salesData =  data.map((item: any) => item.total_purchase_value); 
...
const isfetchSalesPerformanceByProduct = async () => {
	try {
		...
		const response = await httpClient.get(`${import.meta.env.VITE_PUBLIC_SUPPLIER_API_URL}/reporting/supplier-wise-purchase?user_id=${USER_ID}`)
...
<template>
	<div class="p-5 intro-y box h-full">
		<div class="mb-2 flex justify-between items-center">
			<h4 class="text-lg font-semibold text-gray-800 mb-6">{{$t('reporting.SupplierWisePurchaseReport')}}</h4>
			<Button 
				...
				<table class="w-full border-collapse">
					<thead>
						<tr class="bg-gray-50 border-b">
							...
							<th class="text-right py-3 px-4 font-semibold text-gray-700">Total Purchase Value</th>
						</tr>
					</thead>
					<tbody>
						<tr v-for="(item, index) in SalesPerformanceByProduct" :key="index" class="border-b hover:bg-gray-50">
							...
							<td class="py-3 px-4 text-right font-medium text-gray-800">{{ CurrencyFormatter(parseFloat(item.total_purchase_value).toFixed(2)) }}</td>
						</tr>
					</tbody>
				</table>
			</div>
