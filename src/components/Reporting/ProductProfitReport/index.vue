...
const columns = ref([
	{ field: "name_en", title: t('reporting.product_name') },
    { field: "profit", title: t('reporting.profit') },
]);
...
const exportTable = (type: string) => {
  ...
  const filename = 'Product Profit Report';
...
const fetchInvoices = async () => {
	try {
		...
		loading.value = true;
    const response = await httpClient.get(import.meta.env.VITE_PUBLIC_API_URL_REPORTING + "/v1/dashboard/product-profit-loss", {
...
<template>
  <div class="p-5 box" :class="{ 'h-full': fullHeight }">
    <div class="flex justify-between  mb-6">
      <div>
        <h4 class="text-lg font-semibold text-gray-800">{{ $t('reporting.productProfit') }}</h4>
      </div>
