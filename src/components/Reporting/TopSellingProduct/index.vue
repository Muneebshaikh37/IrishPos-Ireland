...
<template>
  <div class="p-5 box h-full">
    <div class="mb-2 flex justify-between items-center">
      ...
      <Button 
        v-if="TopSellingProducts?.label_en?.length > 0 && TopSellingProducts?.total_revenue?.length > 0"
    ...
    <div v-else-if="TopSellingProducts?.label_en.length > 0 && TopSellingProducts?.total_revenue.length > 0">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full border-collapse">
          ...
          <tbody>
            <tr v-for="(label, index) in TopSellingProducts.label_en" :key="index" class="border-b hover:bg-gray-50">
              <td class="py-3 px-4 text-gray-800">{{ label }}</td>
              <td class="py-3 px-4 text-right font-medium text-gray-800">{{ parseFloat(TopSellingProducts.total_revenue[index]).toLocaleString() }}</td>
            </tr>
          </tbody>
        </table>
      </div>
