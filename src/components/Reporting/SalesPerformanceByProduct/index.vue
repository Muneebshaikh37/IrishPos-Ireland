...
const updateChartData = (data: any) => {
  ...
  const salesData =  data.map((item: any) => item.sales_revenue); 
  const xCategories =  data.map((item: any) => item.name_en)
...
const isfetchSalesPerformanceByProduct = async () => {
	try {
		...
		const response = await httpClient.get(`${import.meta.env.VITE_PUBLIC_API_URL_REPORTING}/v1/reporting/sales-performance-by-product?user_id=${USER_ID}&start_date=${start_date.value}&end_date=${end_date.value}`)
...
<template>
	<div class="p-5 intro-y box h-full">
		<div class="mb-2 flex justify-between items-center">
			<h4 class="text-lg font-semibold text-gray-800 mb-6">{{$t('reporting.SalesPerformancebyProduct')}}</h4>
			<div class="flex items-center gap-3">
				<div class="relative mt-3 sm:ml-auto sm:mt-0 text-slate-500 flex items-center" :class="{ 'mr-2': locale === 'ar', '': locale !== 'ar' }">
