...
const columns = ref([
	{ field: "customer_name", title: t('reporting.CustomerName') },
	{ field: "phone", title: t('reporting.phone') },
    { field: "purchase_total", title: t('reporting.purchasetotal') },
...
const exportTable = (type: string) => {
  ...
  const filename = 'Customer Purchase History';
...
const fetchInvoices = async () => {
	try {
		...
		loading.value = true;
    const response = await httpClient.get(import.meta.env.VITE_PUBLIC_API_URL_REPORTING + "/v1/reporting/customer-purchase-history", {
		...
		if (params.search) {
			...
			filteredData = filteredData.filter((item: any) =>
				` ${item.purchase_total} ${item.customer_name} ${item.phone}`
					.toLowerCase()
					.includes(searchTerm)
			);
		}
...
<template>
  <div class="p-5 box" :class="{ 'h-full': fullHeight }">
    <div class="flex justify-between  mb-6">
      <div> 
        <h4 class="text-lg font-semibold text-gray-800">{{ $t('reporting.CustomerPurchaseHistory') }}</h4>
      </div>
