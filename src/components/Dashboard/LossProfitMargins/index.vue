...
const updateChartData = (data) => { 
  ...
  const newData = transformedData.map((item) => item.loss);
...
const isfetchProfitMargins = async () => {
  try {
    ...
    const response = await httpClient.get(`${import.meta.env.VITE_PUBLIC_API_URL_REPORTING}/v1/dashboard/product-loss?user_id=${USER_ID}&start_date=${props.start_date}&end_date=${props.end_date}`);
...
<template>
  <div class="p-5 intro-y box h-full">
    <h4 class="text-lg font-semibold text-gray-800 mb-4">{{ $t('dashboard.TotalLoss') }}</h4>
    <div v-if="isLoading">
