...
const SalesByCategory = ref<any>({ sales: [], labels: [] });
...
const isfetchSalesByCategory = async () => {
  try {
    isLoading.value = true
    const response = await httpClient.get(`${import.meta.env.VITE_PUBLIC_API_URL_REPORTING}/v1/dashboard/top-selling-categories?user_id=${USER_ID}&start_date=${props.start_date}&end_date=${props.end_date}`)
    const result = handleResponse(response);
    if (result.success) {
      SalesByCategory.value = result.data;
      labels.value = Array.isArray(result.data?.labels) ? result.data.labels : [];
      series.value = Array.isArray(result.data?.sales) ? result.data.sales : [];
...
watch([() => props.start_date, () => props.end_date], () => { isfetchSalesByCategory(); });
onMounted(() => {
  isfetchSalesByCategory();
}) 
</script>
<template>
  <div class="p-5 intro-y box h-full">
    <div class="mb-2">
      <h4 class="text-lg font-semibold text-gray-800 mb-6">{{ $t('dashboard.salesByCategory') }}</h4>
    </div>
    <div v-if="isLoading">
      <div class="animate-pulse bg-slate-200 w-full h-56 rounded-lg"> </div>
    </div>
    <div v-else-if="SalesByCategory?.sales?.length > 0 && SalesByCategory?.labels?.length > 0">
