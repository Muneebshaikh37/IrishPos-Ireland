    ...
    <div class="intro-y box p-6 rounded-lg h-fit bg-[#e6ebf0] w-full z-10">
      <div class="flex justify-between mb-4">
        <label class="text-gray-700 font-medium">
          {{ $t('invoices.subtotal') }}
          <span class="text-xs font-medium text-gray-500">({{ $t('invoices.taxExclusive') }})</span>
        </label>
      ...
      <div class="flex justify-between mb-4">
        <label class="text-gray-700 font-medium">
          {{ $t('invoices.total') }}
          <span class="text-xs font-medium text-gray-500">({{ $t('invoices.taxInclusive') }})</span>
        </label>
        <p>{{ formatPrice(total) }}</p>
      </div>
      <div class="mb-4">
        <div class="flex justify-between items-center">
          <label class="text-gray-700 font-medium w-1/2">{{ $t('invoices.amountPaid') }}</label>
          <FormInput
              v-model="SupplierFields.amount_paid"
              type="number"
              min="0"
              class="w-[150px] px-4 py-2 border rounded-lg"
              :class="{'border-red-500': form.invalid('amount_paid')}"
          />
        </div>
      ...
      <div class="flex justify-between mb-4">
        ...
        <div class="flex items-center w-[150px]">
          <TomSelect
              v-model="SupplierFields.payment_type"
              :options="{ placeholder: $t('invoices.selectPayment') }"
              class="w-full bg-white serach-remove"
              :class="{'border-red-500': form.invalid('payment_type')}"
          >
            <option></option>
            <option value="CC">{{ $t('invoices.cc') }}</option>
