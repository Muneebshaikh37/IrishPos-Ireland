<template>
  <div class="mb-4">
    ...
    >
      {{ $t('customers.addWithdrawCash') }}
    </Button>

    <!-- Dialog for Add/Withdraw -->
    <Dialog :open="showTransactionPopup" @close="resetForm">
      <Dialog.Panel>
        <div class="p-5">
          <div class="text-xl text-black my-4 font-medium text-center">{{ $t('customers.addWithdrawAmount') }}</div>
          <div class="pt-2">
            <FormLabel htmlFor="amount">{{ $t('customers.amount') }}* </FormLabel>
            <FormInput
                id="amount"
          ...
          <div class="pt-2">
            <FormLabel htmlFor="notes">{{ $t('customers.notes') }} </FormLabel>
            <FormTextarea
                id="notes"
                v-model="Notes"
...
async function handleTransaction(type) {
  ...
  try {
    ...
    // toast().fry(pan.supplier.error(error.response.data.message))
  } finally {
