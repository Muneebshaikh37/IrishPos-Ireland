<template>
  <div class="mb-4">
    <label for="productSearch" class="block text-gray-700 font-medium mb-2">
      {{ $t('stock-count.productSearch.scanOrTypeProduct') }} <span class="text-danger "> *</span>
    </label>
    <div class="relative">
      <FormInput
          type="text"
          ...
          @input="searchProduct"
          :placeholder="$t('stock-count.productSearch.enterProductCodeOrName')"
          :class="{'border-red-500': form.invalid('products')}"
      />
    ...
    <template v-if="form.invalid('products')">
      <div class="mt-2 text-xs text-red-600">{{ form.getError('products') }}</div>
    </template>
              ...
              <div @click="addProductToInvoice(product)"
                ...
                <div class="flex items-center">
                  ...
                  <div>
                    ...
                    <!-- <p class="text-xs font-light">SKU - 200039</p> -->
                  </div>
                </div>
                <!-- <p class="text-[#F96F01] text-sm">{{ product.price }} SAR</p> -->
              </div>
            </li>
          </ul>
        </div>
        <div v-else-if="productSearch.length" class="mt-4" key="none">
          <p class="text-gray-500 dark:text-gray-400">{{ $t('stock-count.productSearch.noProductsMatched') }}</p>
        </div>
      </div>
    </transition>
  </div>
...
async function searchProduct() {
  ...
  try {
    ...
    if(result.success){
      ...
      matchedProducts.value = result.data.data.filter(product => (
              product.name_en?.toLowerCase().includes(productSearch.value.toLowerCase()) ||
              product.name_ar?.includes(productSearch.value) ||
              product.sku?.includes(productSearch.value) ||
              product.barcode?.includes(productSearch.value)) &&
